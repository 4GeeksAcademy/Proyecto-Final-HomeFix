  ...
  return (
    <div className="max-w-screen-lg mx-auto mt-8">
      <div className="flex space-x-4">
        <button onClick={() => handleTabClick('perfil')}
        ...
        <button onClick={() => handleTabClick('nuevapublicacion')}
                className={`px-4 py-2 rounded focus:outline-none ${activeTab === 'nuevapublicacion' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>
          Subir Publicación
        </button>
        <button onClick={() => handleTabClick('perfil-profesional')}
                className={`px-4 py-2 rounded focus:outline-none ${activeTab === 'perfil-profesional' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>
          Perfil Profesional
        </button>
      </div>
